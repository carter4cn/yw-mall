...
func loadRefundForUpdate(ctx context.Context, tx sqlx.Session, refundId int64, expectStatus int) (*refundRow, error) {
	var r refundRow
	q := "SELECT id, order_id, order_no, user_id, shop_id, amount, reason, evidence, items, status, merchant_user_id, merchant_remark, merchant_handle_time, admin_id, admin_remark, admin_handle_time, appeal_reason, appeal_time, refund_no, refund_complete_time, create_time FROM refund_request WHERE id = ? FOR UPDATE"
...
func loadRefundById(ctx context.Context, svcCtx *svc.ServiceContext, id int64) (*refundRow, error) {
	var r refundRow
	q := "SELECT id, order_id, order_no, user_id, shop_id, amount, reason, evidence, items, status, merchant_user_id, merchant_remark, merchant_handle_time, admin_id, admin_remark, admin_handle_time, appeal_reason, appeal_time, refund_no, refund_complete_time, create_time FROM refund_request WHERE id = ? LIMIT 1"
